...
Como tanto $d$ y $q - p$ son enteros positivos, y el único divisor positivo de $1$ es si mismo, la única posibilidad es que $d = 1$. Entonces, el único divisor común de $n$ y $n + 1$ es $d = 1$, y por lo tanto, $\gcd(n, n + 1) = 1$.
$$\qed$$
